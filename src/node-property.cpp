...
template <typename T> bool check_if_value_changed(Property &prop, const T &v) {
...
#define SET_IMPLEMENTATION(v, emit2)                                           \
  if (!check_if_value_changed(*this, v))                                       \
    return true;                                                               \
  if (impl_set(v)) {                                                           \
    emit valueChanged(v);                                                      \
    emit2;                                                                     \
...
inline void setSpinBoxRange(const Property *prop, QtSpinBox *sb) {
  ...
  if (range) {
    if (range->hasMin())
      sb->setMinimum(static_cast<QtScalar>(range->min));
    if (range->hasMax())
      sb->setMaximum(static_cast<QtScalar>(range->max));
...
template <> struct traits<osgVector2> {
  typedef gui::Vector2Dialog Dialog_t;
};
template <> struct traits<osgVector3> {
  typedef gui::Vector3Dialog Dialog_t;
};
template <> struct traits<osgVector4> {
